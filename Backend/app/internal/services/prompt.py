...
class PromptService:
    ...
    async def callback_handle_raw(self, message: ResultPromptMessage):
        try:
        ...
        if self.ws_manager.is_user_connected(user_id):
            # doesnt work. manager instance in consumer service) AHAHAH
            await self.ws_manager.send_personal_message(user_id, "some")
